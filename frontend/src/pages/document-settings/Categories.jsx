...
export default function Categories() {
    ...
    const getCategories = useCallback(async (page, perPage, search, sort) => {
        try {
            setLoading(true);
            const response = await axiosClient.get("/document-categories", {
                params: {
                    page,
                    per_page: perPage,
                    search,
            ...
            setTotalRecords(response.data.categories.total);
        } catch (err) {
            console.log(err);
    ...
    useEffect(() => {
        getCategories(page, pageSize, debouncedQuery, sortStatus);
    }, [page, pageSize, debouncedQuery, sortStatus, getCategories]);
    ...
    const deleteCategory = (id) => {
        showConfirmAlert(
            ...
            () => deleteCategoryCallback(id),
        );
    };

    const deleteCategoryCallback = async (id) => {
        try {
    ...
    const columns = [
        ...
        { accessor: "category_name", title: "Category Name", sortable: true },
        {
            accessor: "actions",
            title: "Actions",
            render: (cat) => (
                ...
                {loading ? (
                    ...
                    <DataTable
                        ...
                        onRecordsPerPageChange={(n) => {
                            setPageSize(n);
                            setPage(1);
                            getCategories(1, n, debouncedQuery, sortStatus);
                        }}

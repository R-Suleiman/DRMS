...
export default function ServiceRecord() {
    ...
    const getRecord = async () => {
        try {
            const response = await axiosClient.get(`/service-records/${id}`);
                        ...
                        <div className="flex-1 space-y-4">
                            <div>
                                <h1 className="text-3xl font-bold bg-linear-to-r from-slate-800 via-slate-700 to-slate-600 bg-clip-text text-transparent flex items-center gap-2">
                                    {record.service_records?.[0]?.service_name}
                                    {record.service_records?.[0]?.is_classified ? <span className="text-sm font-medium capitalize bg-red-500 text-white px-2 py-1 rounded-md">Classified</span> : <span className="text-sm font-medium capitalize bg-green-500 text-white px-2 py-1 rounded-md">Unclassified</span>}
                                </h1>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div className="group flex items-center gap-3 p-3 rounded-xl bg-linear-to-br from-slate-50 to-slate-50 border border-slate-100/50 hover:shadow-md transition-all duration-200">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-semibold text-slate-700">
                                            {record?.service_records?.[0]?.department?.name}
                                        </p>
                                    </div>
                                </div>
                            </div>
